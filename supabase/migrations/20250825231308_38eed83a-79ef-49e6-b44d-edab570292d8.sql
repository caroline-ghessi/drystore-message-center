...
-- Obter o ID do Ricardo para configurar WHAPI
DO $$
DECLARE
  ricardo_id uuid;
BEGIN
  ...
  IF ricardo_id IS NOT NULL THEN
    -- Inserir configuração WHAPI para Ricardo
    INSERT INTO public.whapi_configurations (
      ...
      seller_id = ricardo_id,
      webhook_url = 'https://groqsnnytvjabgeaekkw.supabase.co/functions/v1/whapi-webhook/' || ricardo_id,
      active = true,
      updated_at = now();
  END IF;
END $$;

-- PASSO 3: Verificar se todas as configurações estão corretas
-- Log de verificação
INSERT INTO public.system_logs (type, source, message, details)
VALUES (
  'info',
  'webhook_setup',
  'Correção completa das configurações de vendedores',
  jsonb_build_object(
