...
DROP POLICY IF EXISTS "operators_can_only_view_unassigned_or_own_conversations" ON public.conversations;
DROP POLICY IF EXISTS "operators_cannot_see_sensitive_seller_data" ON public.sellers;

-- 3. Enhanced phone number masking function
...
-- 5. Add function to log sensitive data access (manual logging)
...
-- 6. Ensure user_id fields are not null where needed
ALTER TABLE public.user_roles ALTER COLUMN user_id SET NOT NULL;

-- 7. Create indexes for better performance on security queries
...
-- 8. Add RLS policies to prevent views from bypassing security
...
-- 9. Create secure wrapper functions for sensitive data access
...
VALUES (
  ...
  'Critical security fixes applied - Phase 1 complete (corrected)',
  jsonb_build_object(
    'fixes_applied', ARRAY[
