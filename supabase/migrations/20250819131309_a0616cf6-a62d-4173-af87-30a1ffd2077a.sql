...
-- 2. Add RLS policies to conversations_with_last_message view
-- First enable RLS on the view
ALTER VIEW public.conversations_with_last_message SET (security_barrier = true);

-- Create RLS policies for conversations_with_last_message
CREATE POLICY "admins_managers_can_view_all_conversations_summary" 
ON public.conversations_with_last_message
FOR SELECT 
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role));

CREATE POLICY "operators_can_view_conversations_summary" 
ON public.conversations_with_last_message
FOR SELECT 
USING (has_role(auth.uid(), 'operator'::app_role));

CREATE POLICY "sellers_can_view_assigned_conversations_summary" 
ON public.conversations_with_last_message
FOR SELECT 
...
-- 3. Add RLS policies to seller_dashboard view
-- Enable security barrier on seller_dashboard
ALTER VIEW public.seller_dashboard SET (security_barrier = true);

-- Create RLS policies for seller_dashboard
CREATE POLICY "admins_managers_can_view_all_seller_metrics" 
ON public.seller_dashboard
FOR SELECT 
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role));

CREATE POLICY "sellers_can_view_own_metrics" 
ON public.seller_dashboard
FOR SELECT 
...
-- 4. Fix database functions security - set proper search_path
...
-- 5. Fix audit_access_attempt function to use new user_id column
...
VALUES (
  ...
  jsonb_build_object(
    'fixes_applied', ARRAY[
      'Added RLS policies to conversations_with_last_message',
      'Added RLS policies to seller_dashboard', 
